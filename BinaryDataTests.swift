...
class BinaryDataTests: XCTestCase {
    ...
    func testNSDataInit() {
        guard let nsData = NSData(base64Encoded: "MTIzNA==", options: NSData.Base64DecodingOptions())
    ...
    func testGetNullTerminatedString() {
        ...
        var bytes = Array(testString.utf8.map{ UInt8($0)})
      bytes.append(0x0)
    ...
    func testPerformanceGetNullTerminatedString() {
        self.measure {
            self.doManyTimes {
                let testString = "TestData"
                let data = BinaryData(data: Array(testString.utf8.map{ UInt8($0)}))
